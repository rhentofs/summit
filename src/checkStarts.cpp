...
int main(int argc, char* argv[]) 
{
  //  cout<<sizeof(int)<<endl;
	...
	while ((maxPos - minPos + 1) - read >= chunk && (chunk > 0))
	  {
	    ...
	    if(first)
	      {
		maxCount = (unsigned int)inpData[0];
		first = false;
	      }
	    ...
	    for (int i = 0; i < chunk; i++)
	      {
		total += (unsigned long)inpData[i];
		if((unsigned int)inpData[i] > maxCount)
		  maxCount = (unsigned int)inpData[i];
	      }
	    read += chunk;
	...
	cout<<(unsigned long)minPos<<"\t"<<(unsigned long)maxPos<<"\t";
	cout<<total<<"\t";
	cout<<maxCount<<endl;
